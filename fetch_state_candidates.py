...
def generate_state_candidates():
    ...
    # FIX: LOAD THE INCUMBENT CROSSWALK GENERATED BY YOUR TAGGER
    incumbent_file = 'state_incumbents.csv'
    INCUMBENTS = set()
    if os.path.exists(incumbent_file):
        inc_df = pd.read_csv(incumbent_file, dtype=str)
        if 'candidate_id' in inc_df.columns:
            INCUMBENTS = set(inc_df['candidate_id'].dropna().str.strip())
            print(f"Successfully loaded {len(INCUMBENTS)} incumbents from crosswalk.")
    else:
        print(f"[!] Warning: {incumbent_file} not found. No incumbent tags will be applied.")

    output_data = []
    
    for _, row in state_df.iterrows():
        acct_num = str(row.get('AcctNum', '')).strip()
        ...
        # Apply the lookups dynamically from your local csv set
        is_inc = 'Y' if acct_num in INCUMBENTS else 'N'
                
        output_data.append({
            'candidate_id': acct_num, 
            'name': full_name,
            'office_full': office_full,
            'party_full': row.get('PartyCode', 'Unknown'), 
            'is_incumbent': is_inc, 
            'receipts': 0.0,
            'disbursements': 0.0,
    ...
    print(f"SUCCESS: Exported {len(final_df)} qualified state candidates with incumbent statuses.")
